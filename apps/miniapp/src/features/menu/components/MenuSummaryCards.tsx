...
import { Layers, Package, PackageX, AlertTriangle, Plus, ChevronRight } from 'lucide-react';
import { useMenuStore } from '../../../store/useMenuStore';
import { ProductAvailabilityEnum } from '@turon/shared';

const MenuSummaryCards: React.FC = () => {
  ...
  const oosProducts = products.filter(p => p.availability === ProductAvailabilityEnum.OUT_OF_STOCK).length;

  const cards = [
    { label: 'Kategoriyalar', value: totalCategories, icon: Layers, color: 'bg-blue-50 text-blue-600', iconBg: 'bg-blue-100' },
    { label: 'Faol taomlar', value: activeProducts, icon: Package, color: 'bg-emerald-50 text-emerald-600', iconBg: 'bg-emerald-100' },
    { label: 'Nofaol', value: inactiveProducts, icon: PackageX, color: 'bg-slate-50 text-slate-500', iconBg: 'bg-slate-100' },
    { label: 'Tugagan', value: oosProducts, icon: AlertTriangle, color: 'bg-red-50 text-red-600', iconBg: 'bg-red-100' },
  ];
