import { Order, OrderStatus, PaymentStatus } from '../../data/types';
import { 
  TimeRange, 
  KPIMetrics, 
...
const statusConfig: Record<OrderStatus, { label: string; color: string }> = {
  [OrderStatus.PENDING]: { label: 'Yangi', color: 'bg-blue-500' },
  [OrderStatus.PREPARING]: { label: 'Tayyorlanmoqda', color: 'bg-amber-500' },
  [OrderStatus.READY_FOR_PICKUP]: { label: 'Tayyor', color: 'bg-orange-500' },
  [OrderStatus.DELIVERING]: { label: 'Yetkazilmoqda', color: 'bg-purple-500' },
  [OrderStatus.DELIVERED]: { label: 'Yetkazib berildi', color: 'bg-emerald-500' },
  [OrderStatus.CANCELLED]: { label: 'Bekor qilingan', color: 'bg-red-500' },
};
