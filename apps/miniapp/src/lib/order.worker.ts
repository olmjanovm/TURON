...
// Kuryerlarni avtomatik biriktirish navbati (Courier Reassignment uchun)
export const courierAssignmentQueue = new Queue('courier-assignment', {
  connection: redis,
...
export const orderWorker = new Worker<OrderJobPayload>(
  ...
  async (job: Job) => {
    ...
    // 3. KURYER BIRIKTIRISH NAVBATIGA QO'SHISH (Auto-Reassignment Logikasi)
    // Buyurtma muvaffaqiyatli DB ga yozilgach, darhol bo'sh kuryer qidirishni boshlaymiz
    await courierAssignmentQueue.add(
      'assign-courier',
      {
        orderId: result.id,
        orderNumber: result.orderNumber,
        excludeCourierIds: [], // Dastlab hech kim qora ro'yxatda emas
      }
    );
